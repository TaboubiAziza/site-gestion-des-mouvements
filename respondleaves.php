...
<head>
  ...
  <title>Respond to Leave demands</title>
...
<body>
  ...
  <section id="container" class="">
    ...
    <header class="header dark-bg">
      ...
      <!--logo start-->
      <a href="index.html" class="logo"><img src="TUTunisair.png"> <span class="lite">Respond to Leave demands</span></a>
      <!--logo end-->
    ...
    <section id="main-content">
      <section class="wrapper">
        <div class="row">
          <div class="col-lg-12">
            <h3 class="page-header"><i class="fa fa-table"></i>Leave Requests</h3>
        ...
        <div class="table-responsive">
          <table class="table  table-striped table-advance table-hover" id="table">
            ...
            <tbody>
              ...
              $req = "SELECT leavee.id, user.matricule, user.name, user.lastname, type_leave.type, leavee.from, leavee.to, leavee.description, leavee.justification, leavee.status FROM leavee, user, type_leave WHERE leavee.matricule = user.matricule AND leavee.id_type_leave = type_leave.id_type_leave  AND user.service = '" . $_SESSION["service"] . "'";
              $myquery = mysqli_query($conn, $req);
              while ($row = mysqli_fetch_array($myquery)) {
                //Difference in days between from-to
                $start = strtotime($row['from']);
                $end = strtotime($row['to']);
                $diff_days = round(($end - $start) / (60 * 60 * 24));
                ...
                <div class='dropdown-menu animated--fade-in' aria-labelledby='dropdownMenuButton' style=''>
                    <a class='dropdown-item btnAccept' href='repondre.php?id=" . $row['id'] . "&rep=1&sender=" . $row['matricule'] . "&diff=" . $diff_days . "'><i class='fas fa-check-double'></i> Accept</a>
                    <a class='dropdown-item btnReject' href='repondre.php?id=" . $row['id'] . "&rep=2'><i class='fas fa-times'></i> Reject</a>
                </div>
                </div>";
                ...
                if ($row['justification'] == NULL) {
                  $img = "No justification";
                } else {
                  $img = "<a href='imagesconge/" . $row['justification'] . "' target='_blank'>Open</a>";
                }

                echo "<tr data-matricule='" . $row['matricule'] . "' data-name='" . $row['name'] . "' data-lastname='" . $row['lastname'] . "'
              data-type='" . $row['type'] . "' data-from='" . $row['from'] . "' data-to='" . $row['to'] . "'
  ...
  <script src="js/respondleave.js"></script>
